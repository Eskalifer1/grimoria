...
| `src/constants/` | Every fixed value the app decides on, one file per subject (`routes.ts`, `action.ts`, `optimistic.ts`, `role.ts`, `theme.ts`, `user.ts`, `app.ts`, `form.ts`, `validation.ts`, `env.ts`, `env.server.ts`, `env.public.ts`) — any layer imports them, they import nothing |

...
| `src/types/` | Ambient module declarations a loader needs (`svg.d.ts` — a script file, because a wildcard module cannot sit in one that imports) |
...
| `src/auth/` | Better Auth plugin objects passed to `betterAuthPlugin`, which `src/payload.config.ts` is the only importer of |
| `src/admin/` | Components Payload's admin renders, reached by string path through the import map |
| `src/constants/` | Every fixed value the app decides on, one file per subject (`routes.ts`, `action.ts`, `optimistic.ts`, `role.ts`, `theme.ts`, `user.ts`, `app.ts`, `form.ts`, `validation.ts`, `env.ts`, `env.server.ts`, `env.public.ts`, `securityHeaders.ts`, `mascot.ts`) — any layer imports them, and they import nothing outside this folder |
...
- **`shared/`** — `components/`, `components/ui/`, `hooks/`, `lib/`, `config/`, `types/`,
  `assets/` (files a loader turns into a module — the mascot SVGs).

...
**Review holds what spans files or branches**, across eleven categories: semantics and structure,
accessible name, keyboard reach and focus movement, ARIA state that tracks real state, forms and
authentication, dynamic content and timing, pointer and target, page level, non-visual cues, locale
markup, and layout resilience. Which Level A and AA criterion falls under which category — and which
are held by a token or a browser instead — is `.claude/skills/a11y-review/checklist.md`, the one
...
- **A live region is mounted before it has anything to say.** A `role="alert"` or `role="status"`
  ...
  One region per announcement.
- **Focus goes to what the change was about, not merely to what survived.** A control that
  disappears under the User — a dismissal, a delete — drops focus to the document body. A message
  about a field returns focus to that field; the nearest surviving ancestor is the fallback for when
  the target went with the change. Focus left on a submit button
  turns the next Enter into another write (WCAG 2.2 AA, 2.4.3).
...
- **A dimmed state still meets contrast.** Opacity multiplies against the background, so a value at
  60% measured 4.02:1 where the token gives 14.36:1. Dim at 70%, and **never dim a message**
  (WCAG 2.2 AA, 1.4.3).

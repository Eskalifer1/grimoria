...
Not everything under `src/` is a layer. These paths are **framework-fixed**: the
framework or a code generator dictates their location, and moving them breaks the
build or costs a config override for nothing.

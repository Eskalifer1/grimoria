...
- **`revalidatePaths` takes `ROUTE_PATTERNS`, never `ROUTES`.** Every page sits under `[locale]`,
  so `/profile` matches no declared route: the call answers success and the screen keeps its stale
  data.

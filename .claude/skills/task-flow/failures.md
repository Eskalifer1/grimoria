...
- **`verify.js` fails to launch or returns nothing** — read `journal.jsonl` in the run's transcript
  directory before re-running; a cached empty result looks identical to a crash.

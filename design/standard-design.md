...
`standard` is the product's default and unflavored face: no wizard framing, no spells. A note
...
broad, extremely soft washes of pale color that never resolve into shapes. Surfaces are white
...
**A family of pale tints** for categorization — soft mint, pale cyan, pale lavender, light
neutral gray — drawn off the two accents plus one neutral, filling tag chips and code blocks.
Close enough in value to read as one system, light enough that dark text sits on them
comfortably. Category color is a label, never decoration.

**A bright cyan for live status only**: a selected edge, an unread marker. The one color
...
body. Not how panels are bounded: a thin gray rectangle around every card is the flatness
...
their centers, so nothing slides sideways when it reopens. As a drawer it is the same surface
...
- **Plain vocabulary only.** A count says what it counts; a navigation row says where it goes.
  Flavored phrasing reads as a mistake here — a note is a note, never an inscription.

...
Both executions are the same figure on a `0 0 256 256` grid, centered on `128 128`.
...
inward to the center. That fold is the crossbar, and it is what makes the figure a G rather than
an ornament. Inside it sits a second, concentric chevron opening to the right, its arms cut
short of the vertical axis.

| | Value |
|---|---|
| Outer ribbon, centerline | `178 78 → 128 28 → 28 128 → 128 228 → 228 128 → 128 128` |
| Inner chevron, centerline | `132 84 → 88 128 → 132 172` |
...
**May differ between executions**: terminals, joins, steadiness of the contour, wear, color.
**May not**: those coordinates. `standard` follows them exactly; `dark-fantasy` offsets the same
centerlines and lets the edge wander within about ±3. **Overlay the two files and the figures
must sit on top of each other** — that is the acceptance test, not a matter of taste.

## `standard`

Round caps and joins, one unbroken run, single flat color. The open ends are pulled back by
...
Color comes from `currentColor`, so one asset serves filled, disabled, and print.
...
- **Each is centered on a contour**, not inside the stroke, and clipped to the mark — so half
  falls off the ink and what remains bites into the edge. Centered inside the stroke they become
...
- **They darken.** Worn ink moves toward its ground; the reference's grays lighten only because
...
**Placement is finicky, and worth knowing before nudging one.** A patch centered a few units
short of a vertex leaves a thin wedge of ink beyond it and reads as a facet cut off the corner;
one centered a few units past it is clipped away entirely. **The window is about 2 units wide**,
and the filled area of these traced shapes is not centered in their bounding box, so it is found
...
Color comes from CSS custom properties rather than `currentColor`, since three roles cannot
follow one inherited color: `logo-ink` `#C0453C`, `logo-edge` `#FFB4AB` (`--blood-light`
unchanged), `logo-scuff` `#6B231D`. Each patch keeps the reference's hierarchy through its
opacity. Two consequences: the two files are parameterized differently, and inlining this one
...
- Be recolored into the other theme's palette. Teal is `standard`, red ink is `dark-fantasy`;
...
strokes — and neither is kept in the repo. The skeleton above is the normalized redraw and wins

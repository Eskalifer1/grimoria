...
- **Needing a concrete colour, radius, shadow or duration**: `src/styles/standard.css` and `src/styles/dark-fantasy.css` — the values themselves, and the only place they exist
- **Changing a token value, or wondering why one is what it is**: `design/standard-tokens.md` and `design/dark-fantasy-tokens.md` — contrast fixes, accent-role rules, font weights to load, type scales, and what is deliberately unsettled. Read before retuning a value; they hold no values of their own
...
- **Adding or renaming a design token**: `design/token-contract.md` — the semantic names both Themes fill. A token is added to both Themes in the same change, or not at all; a component that branches on the active Theme means the contract is missing a name
...
- **Building any React component** — server/client boundary, state placement, splitting one that grew, loading and error states: `docs/agents/coding-standards/components.md`. `views/` never carries `"use client"`, a server module carries `server-only`, and JSX conditionals close on `null` because `&&` renders `0`
- **Writing styles in code** — Tailwind utilities, tokens, shadcn primitives, variants: `docs/agents/coding-standards/styling.md`. The `design/` docs say what a surface looks like; this one says how code is allowed to say it. Colours, radii, shadows, fonts and easings outside the tokens do not compile, spacing is Tailwind's own scale, and a component never branches on the active Theme
